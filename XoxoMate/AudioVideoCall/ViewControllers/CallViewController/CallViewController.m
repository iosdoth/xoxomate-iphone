...
- (NSNumber *)currentUserID {
    ...
    return @(UsersDataSourceCall.instance.currentUser.ID);
}
...
- (void)viewDidLoad {
    ...
    if (self.session.conferenceType == QBRTCConferenceTypeVideo) {
        ...
        self.cameraCapture = [[QBRTCCameraCapture alloc] initWithVideoFormat:settings.videoFormat position:settings.preferredCameraPostion];
        [self.cameraCapture startSession];
    }
    
    QBUUser *initiator = [ServicesManager instance].currentUser;

    _isOffer = [UsersDataSourceCall.instance.currentUser isEqual:initiator];
    ...
    [users insertObject:UsersDataSourceCall.instance.currentUser atIndex:0];
    
     NSMutableArray *opponents = [[NSMutableArray alloc]init];
    
    if (self.isIncoming) {
        NSLog(@"Session %@",self.session.opponentsIDs);
        NSString *string = [NSString stringWithFormat:@"%@",[self.session.opponentsIDs objectAtIndex:0]];
        NSUInteger intvalue = [string integerValue];
        
        QBUUser *user = [[QBUUser alloc]init];
        user.ID = intvalue;
        [opponents addObject:user];
        
    }else{
        opponents = [UsersDataSourceCall.instance usersWithIDSWithoutMe:self.session.opponentsIDs].mutableCopy;
    }
...
- (UIView *)videoViewWithOpponentID:(NSNumber *)opponentID {
    ...
    if (UsersDataSourceCall.instance.currentUser.ID == opponentID.integerValue) {//Local preview
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@">>>>>user id :: %lu",(unsigned long)user.ID);
    
    [cell setVideoView:[self videoViewWithOpponentID:@(user.ID)]];
    NSString *markerText;
    
    if (indexPath.row == 0) {
        markerText = [NSString stringWithFormat:@"%lu", (unsigned long)0 + 1];
        NSLog(@"Inside if condition");
    }else{
        markerText = [NSString stringWithFormat:@"%lu", (unsigned long)1 + 1];
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
    [self.opponentsCollectionView performBatchUpdates:nil completion:nil];
}
...
- (NSIndexPath *)indexPathAtUserID:(NSNumber *)userID {
    if (UsersDataSourceCall.instance.currentUser.ID == userID.integerValue){
