...
- (void)viewDidLoad {
    ...
    self.users = UsersDataSourceCall.instance.usersWithoutMe;
    ...
    self.title = [NSString stringWithFormat:@"Logged in as %@", UsersDataSourceCall.instance.currentUser.fullName];
}
...
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section {
    ...
    if([view isKindOfClass:[UITableViewHeaderFooterView class]]){
        ...
        tableViewHeaderFooterView.textLabel.text = @"Select users you want to call";
    }
}
...
- (void)callWithConferenceType:(QBRTCConferenceType)conferenceType {
    ...
    if ([self usersToCall]) {
        ...
        NSArray *opponentsIDs = [UsersDataSourceCall.instance idsWithUsers:self.selectedUsers];
        //Create new session
...
- (BOOL)usersToCall {
    ...
    if (!isOK) {
        ...
        [SVProgressHUD showErrorWithStatus:@"Please select one or more users"];
    }
