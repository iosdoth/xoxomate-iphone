...
- (void)refreshMessagesShowingProgress:(BOOL)showingProgress {
    ...
    if (showingProgress) {
        [SVProgressHUD showSuccessWithStatus:@"Loading messages"];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.lblName.text = self.dialog.name;
                      ...
                      date:(NSDate *)date
{
    ...
    message.senderNick = [ServicesManager instance].currentUser.login;
...
- (NSAttributedString *)topLabelAttributedStringForItem:(QBChatMessage *)messageItem {
    ...
    NSString *topLabelText = self.dialog.name;// self.opponentUser.fullName != nil ? self.opponentUser.fullName : self.opponentUser.login;
...
- (NSAttributedString *)bottomLabelAttributedStringForItem:(QBChatMessage *)messageItem {
    ...
    if ([messageItem senderID] == self.senderID) {
        if (messageItem.attachments.count > 0) {
           text = [NSString stringWithFormat:@"%@\nSeen : %@", text, self.dialog.name];
        } else {
            text = [NSString stringWithFormat:@"%@\nRead : %@", text, self.dialog.name];
        }
    }
...
- (void)callWithConferenceType:(QBRTCConferenceType)conferenceType {
    ...
    NSString *receipentID = [NSString stringWithFormat:@"%ld",self.dialog.recipientID];
    
//    NSArray *opponentsIDs = @[receipentID];
...
//    QBRTCSession *session = [QBRTCClient.instance createNewSessionWithOpponents:opponentsIDs withConferenceType:conferenceType];
    ...
    if (session) {
        ...
        callViewController.isIncoming = NO;
